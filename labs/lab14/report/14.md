...
Создаю поддиректорию ~/work/os/lab14, в ней создаю и заполняю файлы common.h, service.c, client.c (рис. @fig:001): 

![Создание файлов common.h, service.c, client.c ](image/1.png){ #fig:001 width=70% }
...
Заполняю файл server.c, реализующий сервер. Использую функцию clock для определения времени работы сервера. Сервер работает не бесконечно, а прекращает работу через некоторое время, в моем случае, через 30 секунд (рис. @fig:003):
 
![Содержимое файла реализации сервера](image/3.png){ #fig:003 width=70% }


Заполняю файлы client.c (рис. @fig:004):

![ Содержимое файла реализации клиента ](image/4.png){ #fig:004 width=70% }

Создаю и заполняю Makefile (рис. @fig:005): 

![Содержание Makefile.](image/5.png){ #fig:005 width=70% }

Затем выполняю программу. На одной консоли запускаю программу server, а на другой консоли запускаю программу client (рис. @fig:006).

![Запуск программ на двух консолях. ](image/6.png){ #fig:006 width=70% }
